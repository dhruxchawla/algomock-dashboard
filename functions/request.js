...
export async function login(email, password) {
  ...
  try {
    let res = await axios.post(baseUrl + url["auth.login"], raw, {
      headers: header,
    });
    return res;
  } catch (err) {
...
export async function register(name, email, password) {
  ...
  try {
    let res = await axios.post(baseUrl + url["auth.register"], raw, {
      headers: header,
    });
    return res;
  } catch (err) {
...
export default async function setUpAccount(accessToken, payload, brokerName) {
  try {
    let res = await axios.post(
      baseUrl + url[`${brokerName}.setupAccount`],
      payload,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
...
export async function deleteBrokerAccount(accessToken, broker_name, broker_id) {
  try {
    let res = await axios.delete(
      baseUrl + "broker/" + broker_name + "/accounts/" + broker_id,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
...
export async function createstrategy(algoName){
    var raw = JSON.stringify({algoName});
    try{
        let res = await axios.post(baseUrl + url['strategy.create'], raw, {headers : header});
        return res;
    }catch(err){
        console.log("error : ", err);
        return err?.response?.data;
